...
export function buildCourseLessonOutcomeData( props ) {
    ...
    let { lessons, courseOutcomes, outcomeInsights, selectedMultiSelectFormTypeOptions, lessonTitle, outcomeId, setSelectedTooltipItemLesson } = props; 
        ...
        lessons.forEach( ( lesson, index ) => {
            ...
            if ( lesson?.title === lessonTitle ) {  
                ...
                setSelectedTooltipItemLesson( lesson );

                let labels = getLessonLabelTitle( lesson?.title );
                let id = getLessonLabelTitle( lesson?.title );
                ...
                lessonOutcomes.forEach(( outcome, index ) => { 
                    ...
                    if ( outcome?._id === outcomeId ) {
                        ...
                        let rate = getCoursePassRateCollection( totalNumberOfquestionsInOutcome, totalNumberOfQuestionsPassedOutcome );
                            ...
                            if ( rate && typeof(rate) === 'number' && currentOutcomeInsight?.length > 0 ) {

                                collection.push({
                                    type: 'bar',
                                    labels,
                                    fill: false,
                                    label: getLessonOutcomeLabelTitle( lessonType?.value ), //outcome?.title
...
function getLessonLabelTitle( lessonTitle ) {
    return `${restrictTextLength( lessonTitle, 15, 15 )}`;
}

function getLessonOutcomeLabelTitle( title ) {
    return capitalizeFirstLetterOfString( title );
}
...
function assignValueToArray( iterator, value ) {
    ...
    if ( typeof( iterator ) === 'number' && typeof( value ) === 'number' ) {
        ...
        // collection.splice(iterator, 1, value) // don't want to change this...
