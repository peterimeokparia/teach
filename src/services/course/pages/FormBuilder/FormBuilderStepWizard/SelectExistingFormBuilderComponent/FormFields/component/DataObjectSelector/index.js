...
const DataObjectSelector = ( { 
    ...
    let dropDownsExist = dropDownOptions?.length > dropDownValues?.length;
    ...
    }, [ dropDownsExist, dropDownOptions, dropDownValues, formFieldElement, saveFormField  ]);
    ...
    }, [ input?.target?.value, courses, formFieldElement, lessons, saveFormField, students, tutors ]);
    ...
    const handleDropDownSelection = ( value ) => {
       if ( !currentUser?._id ) return;
            setInputValue( value );
        if ( value && formBuilderState === elementMeta.state.Manage  ) {
            saveFormField( { ...formFieldElement, answerKey: value, inputValue: value} );
        }
        if ( value && formBuilderState === elementMeta.state.Taking ) {
    ...
    { ( dropDownValues?.length > 0 ) &&  
        <div>
         <span>
            <select 
                name={"select"}
                value={ formFieldElement?.inputValue ? formFieldElement?.inputValue : (( studentsAnswers !== "" || studentsAnswers !== null ) ? studentsAnswers : inputValue) }
