...
const LessonPlanInviteUserVerification = ({ 
  ...
  useEffect(() => {
    loadUsers();
    if ( userCredentials?.existingUser === userExist.Yes ) {
    ...
    if ( userCredentials?.existingUser === userExist.No ) {
        setUserCredentials( {...userCredentials, existingUser: userExist.Yes } );
        createUser({
          ...new SiteUser(), 
